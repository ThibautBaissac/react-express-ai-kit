...
- **The ORM boundary.** This starter chooses Drizzle with SQLite, but the data layer stays behind repository interfaces. The `.claude/` pack itself is ORM-adaptive: the schema/migration subagent detects whatever ORM your target repo uses (Prisma, Drizzle, Mongoose, Knex, TypeORM, raw SQL, …).
...
| ------------------------- | ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `architecture.md`         | _(always on)_                                                                                          | KISS/DRY/SRP/YAGNI, vertical slices, shared-layer placement, one-way layering, parse-don't-validate, "don't abstract until it earns it". |
