 ...
 *
 * Usage: tsx scripts/complete-plan.ts <taskId>
 */
...
function completePlan(taskId: string | undefined): void {
  // Validate taskId
  if (!taskId) {
  ...
  if (status.planificationComplete) {
    console.log(
      `${colors.cyan}Info:${colors.reset} Task ${parsedTaskId} planification is already marked as complete`,
    );
    process.exit(0);
  }
  ...
  try {
    status.planificationComplete = true;
    status.updatedAt = new Date().toISOString();
    writeStatus(status);

    console.log("");
    console.log(
      `${colors.green}${colors.bright}Planification marked as complete!${colors.reset}`,
    );
